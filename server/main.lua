...
RegisterCommand('gme', function(source, args, rawCommand)
    ...
    TriggerClientEvent('cc-rpchat:addMessage', -1, '#f39c12', 'fa-solid fa-person', 'Me (Global) | '..playerName, msg)
end, false)

-- Global Do
RegisterCommand('gdo', function(source, args, rawCommand)
    local playerName
    local msg = rawCommand:sub(5)
    if config.emoji.gdo then
        for _, em in ipairs(emoji) do
            for _, code in ipairs(em[1]) do
                local emojiCode = string.gsub(code, "([%^%$%(%)%%%.%[%]%*%+%-%?])", "%%%1") -- Escape special characters in the emoji code
                msg = string.gsub(msg, emojiCode, em[2]) -- Replace the emoji code with the corresponding emoji character
